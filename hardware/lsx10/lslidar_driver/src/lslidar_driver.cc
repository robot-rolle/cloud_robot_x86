...
namespace lslidar_driver
{
	...
	void LslidarDriver::initParam() // 初始化驱动参数
	{
		...
		ROS_INFO("truncated_mode_: %d", truncated_mode_);
		ROS_INFO("disable_min: %d", disable_angle_min_range[0]);
		ROS_INFO("disable_angle_max_range: %d", disable_angle_max_range[0]);
		if (lidar_name == "M10")
		{
			use_gps_ts = false;
			PACKET_SIZE = 92;
