...
namespace move_base {
  ...
  MoveBase::MoveBase(tf2_ros::Buffer& tf) :
    tf_(tf),//坐标变换TF2的接口对象；
    as_(NULL),//Action服务器；
    planner_costmap_ros_(NULL), controller_costmap_ros_(NULL),//前者是用于全局规划器的代价地图对象，后者是局部规划器所用的代价地图对象；
    bgp_loader_("nav_core", "nav_core::BaseGlobalPlanner"),//装载全局规划器的插件；
    blp_loader_("nav_core", "nav_core::BaseLocalPlanner"),//装载局部规划器的插件；
    recovery_loader_("nav_core", "nav_core::RecoveryBehavior"),
    planner_plan_(NULL), latest_plan_(NULL), controller_plan_(NULL),//三个记录规划结果的缓存；
    runPlanner_(false), setup_(false), p_freq_change_(false), c_freq_change_(false), new_global_plan_(false){//控制和反映MoveBase系统状态的布尔变量；
    ...
    private_nh.param("controller_patience", controller_patience_, 10.0);
    ...
    local_planner_thread_ = new boost::thread(boost::bind(&MoveBase::localPlanThread,this));
    ...
    planner_costmap_ros_->start();       //costmap2dROS start
    ...
    if(shutdown_costmaps_){
      ...
      // bool loadstate_ = !loadRecoveryBehaviors(private_nh);
    //load any user specified recovery behaviors, and if that fails load the defaults
    if(!loadRecoveryBehaviors(private_nh)){
      ROS_INFO(" used the default ");
...
---------------------------------------*/
  ...
  MoveBase::~MoveBase(){
    ...
    local_planner_thread_->interrupt();

    planner_thread_->join();
    local_planner_thread_->join();
    
    delete planner_thread_;
    delete local_planner_thread_;
  ...
  void MoveBase::localPlanThread()
  {
      boost::unique_lock<boost::recursive_mutex> lock(planner_mutex_);
      ros::Timer LocalTime_;
  ...
  void MoveBase::planThread(){
    ...
    //使用unique_lock  枷锁一个boost::recursive_mutex 类型的 全局变量 planner_mutex_
    boost::unique_lock<boost::recursive_mutex> lock(planner_mutex_);
    while(n.ok()){
      // 确认是否要运行路径规划器(这里已经加锁)
      ...
      // 该开始规划了，复制路径规划器的目标点(注意这里在上次循环中加锁了)，然后在这次解锁。P
  ...
  void MoveBase::executeCb(const move_base_msgs::MoveBaseGoalConstPtr& move_base_goal)
  {
    ...
    lock.lock();
    runPlanner_ = true;
    planner_cond_.notify_one(); //进入到 planthread 
    lock.unlock();
...
bool MoveBase::executeCycle(geometry_msgs::PoseStamped& goal){
  ...
  /**
  ** @name:              
  ** @brief:           false used the default true use the recovery
  ** @param:             
  ** @date:              2024-09-25
  ** @version:           V0.0
  ---------------------------------------*/
  bool MoveBase::loadRecoveryBehaviors(ros::NodeHandle node){
    //XmlRpc::XmlRpcValue behavior_list   使用xmlrpc 协议 进程间调用
  ...
  void MoveBase::loadDefaultRecoveryBehaviors(){
    ...
    try{
      ...
      if(clearing_rotation_allowed_){
        ROS_INFO("________reset rotate_recovery");
        rotate->initialize("rotate_recovery", &tf_, planner_costmap_ros_, controller_costmap_ros_);
        recovery_behavior_names_.push_back("rotate_recovery");
        ROS_INFO("step second rotate_recovery");
