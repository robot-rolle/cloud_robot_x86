...
void GlobalPlanner::initialize(std::string name, costmap_2d::Costmap2D* costmap, std::string frame_id) {
    if (!initialized_) {
        ...
        scan_sub_ = private_nh.subscribe<sensor_msgs::LaserScan>("/scan",1000,&GlobalPlanner::ScanCallback,this);
        ...
        private_nh.param("planner_mode",planner_mode_,1);
        
        // private_nh.param("car_mode", car_mode_,);
        //param with robot 
        // test rrt接口
...
bool GlobalPlanner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,
    ...
    double wx = start.pose.position.x; double wy = start.pose.position.y;
    ...
    // ROS_INFO(" the length is [%d]",plan.size());
    OldTime =NewTime;
    // 发布路径和可视化
    publishPlan(plan);
...
bool GlobalPlanner::getPlanFromPotential(double start_x, double start_y, double goal_x, double goal_y,
    ...
    for (int i = path.size() -1; i>=0; i--) { //依次存储路径point 不存储方向 
...
void  GlobalPlanner::publishPotential(float* potential)
{
        int nx = costmap_->getSizeInCellsX(), ny = costmap_->getSizeInCellsY();
        double resolution = costmap_->getResolution();
        nav_msgs::OccupancyGrid PotentialGrid_;
        PotentialGrid_.header.frame_id = frame_id_;
        PotentialGrid_.header.stamp = ros::Time::now();
        PotentialGrid_.info.resolution = resolution;
        PotentialGrid_.info.width = nx;
        PotentialGrid_.info.height = ny;

        double widthx_,widthy_;
        costmap_->mapToWorld(0,0,widthx_,widthy_);
        PotentialGrid_.info.origin.position.x = widthx_ - resolution/2;
        PotentialGrid_.info.origin.position.y = widthy_ - resolution/2;
        PotentialGrid_.info.origin.position.z = 0.0;
        PotentialGrid_.info.origin.orientation.w = 1.0;
        PotentialGrid_.data.resize(nx*ny);
        float max = 0.0;
        for(unsigned int i=0; i<PotentialGrid_.data.size();i++)
        {
        ...
        for(unsigned int i =0; i<PotentialGrid_.data.size();i++)
        {
            ...
            { PotentialGrid_.data[i] = -1;}
            else{
                 if(fabs(max)<DBL_EPSILON) 
                    { PotentialGrid_.data[i] = -1;}
                 else
                    { PotentialGrid_.data[i] = potential_array_[i]*publish_scale_/max;}
            }
        }
        potential_pub_.publish(PotentialGrid_);
}
...
** @version:           V0.0
---------------------------------------*/
void GlobalPlanner::ScanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    int count = scan->scan_time / scan->time_increment; //输出
    std::vector<std::pair<float, float>> Angle_Distance;
    std::pair<float, float> ScanP_;
    // ROS_INFO("I heard a laser scan %s[%d]:", scan->header.frame_id.c_str(), count);
    // ROS_INFO("angle_range, %f, %f", RAD2DEG(scan->angle_min), RAD2DEG(scan->angle_max));
    for(int i = 0; i < count; i++) {
        float degree = RAD2DEG(scan->angle_min + scan->angle_increment * i);
        ScanP_.first = degree;
        ScanP_.second = scan->ranges[i];
        if((i>254) && (i<1281))
        {
          Angle_Distance.push_back(ScanP_);
         if(Angle_Distance[i-255].second<1.2)
         { 
            // ROS_INFO(" the angle is [%f]",Angle_Distance[i-255].first);  //存入最大的障碍快 提取路径
         }
