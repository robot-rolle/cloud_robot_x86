...
** @param:             sensor_msgs::LaserScan::conster msg1 msg2
** @date:              2024-12-11
** @version:           V0.0
---------------------------------------*/
void callback(const sensor_msgs::LaserScan::ConstPtr &msg1,
  ...
  {   
      ...
      newPointAngle = msg1->angle_min + msg1->angle_increment * i;//计算当前角度
      if(truncated_mode_==1)//如果开启截断模式
      {
          int flag_truncate =0;//截断标志
          ...
          if(flag_truncate)continue;//如果当前角度在截断范围内，则跳过
      }
      x = msg1->ranges[i] * cos(newPointAngle);//
      y = msg1->ranges[i] * sin(newPointAngle);
      newPoint.x = x*cos(DEG2RAD(lidar1_rotate))-y*sin(DEG2RAD(lidar1_rotate))+float(lidar1_xoffset);//计算点云的x坐标
      newPoint.y = x*sin(DEG2RAD(lidar1_rotate))+y*cos(DEG2RAD(lidar1_rotate))+float(lidar1_yoffset);//计算点云的y坐标
      newPoint.intensity = msg1->intensities[i];//赋值点云的强度
      pointcloud.push_back(newPoint);//将点云添加到点云数组中
  }


  angle_max = msg2->angle_max;//
  angle_min = msg2->angle_min;//
  angle_increment = msg2->angle_increment;
  node_count = round((angle_max-angle_min)/angle_increment);
  //将scan2的消息转换为点云
  for (int i=0;i<node_count;i++)//
  {   
      ...
      x = msg2->ranges[i] * cos(newPointAngle);//计算点云的x坐标
      y = msg2->ranges[i] * sin(newPointAngle);//计算点云的y坐标
      newPoint.x = x*cos(DEG2RAD(lidar2_rotate))-y*sin(DEG2RAD(lidar2_rotate))+lidar2_xoffset;//计算点云的x坐标
      newPoint.y = x*sin(DEG2RAD(lidar2_rotate))+y*cos(DEG2RAD(lidar2_rotate))+lidar2_yoffset;//计算点云的y坐标
      newPoint.intensity = msg2->intensities[i];//赋值点云的强度
...
int main(int argc, char **argv) {
  ...
  // 分别订阅两个topic
  // 创建两个LaserScan类型的订阅者，用于接收两个激光雷达的数据
// 参数说明：节点句柄、话题名、队列大小、传输选项(使用TCP无延迟模式)
